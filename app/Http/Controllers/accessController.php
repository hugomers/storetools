...
public function getInvoices(Request $request){
        $res = [];
        $invoices = array_map(function($val){return "'".$val."'";},$request->invoices);
        ...
        foreach($facturas as &$factura){
            // if($fac){
                ...
                F_LFA.TOTLFA AS TOTAL
                FROM  F_LFA
                WHERE F_LFA.TIPLFA&'-'&FORMAT(F_LFA.CODLFA,'000000') = "."'".$factura['FACTURA']."'";
                $exec = $this->conn->prepare($sqline);
                $exec->execute();
                $lines = $exec->fetchall(\PDO::FETCH_ASSOC);
    ...
    public function getEntries(Request $request){
        ...
        // $invoices = $request->invoices;
            $sql = "SELECT
                ...
                F_FRE.OB1FRE AS OBSERVACION
            FROM F_FRE
            WHERE F_FRE.TIPFRE&'-'&FORMAT(F_FRE.CODFRE,'000000') IN ($strinvoice)";
            // return $sql;
            $exec = $this->conn->prepare($sql);
            $exec->execute();
            $facturas = $exec->fetchall(\PDO::FETCH_ASSOC);
        ...
        foreach($facturas as &$factura){
            // if($fac){
                ...
                F_LFR.TOTLFR AS TOTAL
                FROM F_LFR
                WHERE F_LFR.TIPLFR&'-'&FORMAT(F_LFR.CODLFR,'000000') = "."'".$factura['FACTURA']."'";
                $exec = $this->conn->prepare($sqline);
                $exec->execute();
                $lines = $exec->fetchall(\PDO::FETCH_ASSOC);
                $factura['products'] = $lines;
