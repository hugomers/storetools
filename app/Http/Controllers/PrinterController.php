...
class PrinterController extends Controller
{
    public function printck($order,$cash,$folio,$config){
        ...
        try {
            try{
                ...
                $printer->text("Escanea el codigo para brindrate\n");
                $printer->text("un mejor servicio\n");
                $printer->text("\n");
    ...
    public function printCount($header){
        $connector = new NetworkPrintConnector($header['print'], 9100, 3);
        // $connector = new NetworkPrintConnector('192.168.10.100', 9100, 3);
        if($connector){
            $printer = new Printer($connector);
            $printer->text(" \n");
            $printer->text(" \n");
