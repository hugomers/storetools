...
class ProductsController extends Controller
{
    ...
    public function regisprice(Request $request){
        ...
        foreach($prices as $price){
            ...
            if($codver){
                ...
                $aaa = isset($price['AAA']) ? round($price['AAA'],2) : intval($codver['AAA']);
                        ...
                        if($centro <= $especial){
                            if($especial <= $caja){
                                if($caja <= $docena){
                                    if($docena <= $mayoreo){
                                        if($mayoreo <= $menudeo){
                                            $costoupd = $this->conn->prepare("UPDATE F_ART SET PCOART = ? WHERE CODART = ?")->execute([$costo,$codigo]);
                                            $cosms = DB::connection('vizapi')->table('products')->where('code',$codigo)->update(['cost'=>$costo]);
                                            $aaaupd = $this->conn->prepare("UPDATE F_LTA SET PRELTA = ? WHERE ARTLTA = ? AND TARLTA = ?")->execute([$aaa,$codigo,7]);
                                            $aaams = DB::connection('vizapi')->table('product_prices as PP')->join('products as P','P.code','PP._product')->where('P.code',"'".$codigo."'")->update(['PP.price'=>$aaa,'PP._type'=>7]);
                                            $centroupd = $this->conn->prepare("UPDATE F_LTA SET PRELTA = ? WHERE ARTLTA = ? AND TARLTA = ?")->execute([$centro,$codigo,6]);
                                            $centroms = DB::connection('vizapi')->table('product_prices as PP')->join('products as P','P.code','PP._product')->where('P.code',"'".$codigo."'")->update(['PP.price'=>$centro,'PP._type'=>6]);
                                            $especiaupd = $this->conn->prepare("UPDATE F_LTA SET PRELTA = ? WHERE ARTLTA = ? AND TARLTA = ?")->execute([$especial,$codigo,5]);
                                            $especialms = DB::connection('vizapi')->table('product_prices as PP')->join('products as P','P.code','PP._product')->where('P.code',"'".$codigo."'")->update(['PP.price'=>$especial,'PP._type'=>5]);
                                            $cajaupd = $this->conn->prepare("UPDATE F_LTA SET PRELTA = ? WHERE ARTLTA = ? AND TARLTA = ?")->execute([$caja,$codigo,4]);
                                            $cajams = DB::connection('vizapi')->table('product_prices as PP')->join('products as P','P.code','PP._product')->where('P.code',"'".$codigo."'")->update(['PP.price'=>$caja,'PP._type'=>4]);
                                            $docenaupd = $this->conn->prepare("UPDATE F_LTA SET PRELTA = ? WHERE ARTLTA = ? AND TARLTA = ?")->execute([$docena,$codigo,3]);
                                            $docenams = DB::connection('vizapi')->table('product_prices as PP')->join('products as P','P.code','PP._product')->where('P.code',"'".$codigo."'")->update(['PP.price'=>$docena,'PP._type'=>3]);
                                            $mayoreoupd = $this->conn->prepare("UPDATE F_LTA SET PRELTA = ? WHERE ARTLTA = ? AND TARLTA = ?")->execute([$mayoreo,$codigo,2]);
                                            $mayoreoms = DB::connection('vizapi')->table('product_prices as PP')->join('products as P','P.code','PP._product')->where('P.code',"'".$codigo."'")->update(['PP.price'=>$mayoreo,'PP._type'=>2]);
                                            $menudeoupd = $this->conn->prepare("UPDATE F_LTA SET PRELTA = ? WHERE ARTLTA = ? AND TARLTA = ?")->execute([$menudeo,$codigo,1]);
                                            $menudeoms = DB::connection('vizapi')->table('product_prices as PP')->join('products as P','P.code','PP._product')->where('P.code',"'".$codigo."'")->update(['PP.price'=>$menudeo,'PP._type'=>1]);
        ...
        //falta poner lo de actualizar el modelo a el dia de modificacion
        $res = [
            "actualizados"=>$actualizados,
            "stores"=>$stor,
            "mysql"=>$mysql,
    ...
    public function regispricesstores(){

    }

    public function regispricepub(){
